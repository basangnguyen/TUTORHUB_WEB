...
// AudienceMutationDigest returns a keyed, tenant-independent digest for a
// canonical audience replacement payload. It is intended for idempotency
// receipts only; callers must never expose it as an API value or log the
// payload. A dedicated sub-key prevents audience request fingerprints from
// being confused with recipient lookup or RSVP capability digests.
func (protector *Protector) AudienceMutationDigest(value []byte) ([sha256.Size]byte, error) {
	...
	mac := hmac.New(sha256.New, protector.audienceKey[:])
	_, _ = mac.Write(lengthDelimitedBytes(
		[]byte(audienceMutationDomain),
...
// RSVPCapabilityTokenDigest returns the versioned, keyed lookup digest for a
// raw RSVP capability token. The raw token must be generated by a CSPRNG and
// is never persisted. A dedicated sub-key and domain keep capability lookup
// independent from recipient deduplication and encrypted calendar payloads.
//
...
func (protector *Protector) RSVPCapabilityTokenDigest(
	rawToken []byte,
...
func deriveAudienceMutationKey(key []byte) [sha256.Size]byte {
	return deriveKeyWithDomain("tutorhub.calendar.audience-mutation.key.v1", key)
}
