...
func TestFeatureControlHandlersHideCrossTenantAndReplaceTypedAggregate(t *testing.T) {
	...
	body := `{"expected_version":3,"features":{"membership_invitations":false,"class_management":true,"class_invite_links":false,"class_session_scheduling":true,"class_session_recurrence":true,"in_app_notifications":false},"quotas":{"members":90,"active_classes":20,"invite_creations_per_hour":40}}`
	request := httptest.NewRequest(
		http.MethodPut,
	...
	if service.putContext.TenantID != tenantID || service.putInput.ExpectedVersion != 3 ||
		len(service.putInput.FeatureOverrides) != 6 || len(service.putInput.QuotaOverrides) != 3 {
...
func TestFeatureControlUpdateRejectsMissingAggregateFields(t *testing.T) {
	...
	for name, body := range map[string]string{
		"expected version": `{"features":{"membership_invitations":true,"class_management":true,"class_invite_links":true,"class_session_scheduling":true,"class_session_recurrence":true,"in_app_notifications":true},"quotas":{"members":100,"active_classes":25,"invite_creations_per_hour":60}}`,
		...
		"quotas":           `{"expected_version":0,"features":{"membership_invitations":true,"class_management":true,"class_invite_links":true,"class_session_scheduling":true,"class_session_recurrence":true,"in_app_notifications":true}}`,
		"quota value":      `{"expected_version":0,"features":{"membership_invitations":true,"class_management":true,"class_invite_links":true,"class_session_scheduling":true,"class_session_recurrence":true,"in_app_notifications":true},"quotas":{"members":100,"active_classes":25}}`,
...
func completeFeatureCapabilities(tenantID uuid.UUID) featurecontrol.Capabilities {
	...
	return featurecontrol.Capabilities{
		...
		Features: []featurecontrol.FeatureCapability{
			...
			{EffectiveFeature: featurecontrol.EffectiveFeature{Key: featurecontrol.FeatureClassSessionRecurrence, Enabled: true}, ConfiguredEnabled: true},
