...
| Trạng thái gần nhất   | P5-COLLAB-18 `DONE`; exact internal canary, recovery và cleanup PASS                         |
...
Playwright/axe, public privacy headers và Secret scan PASS. Authenticated live role/API
safety-admin matrix và manual NVDA `2026.1.1.55980` trên organizer/public production route
cũng PASS; task chuyển `VERIFY -> DONE` ngày 2026-08-03. P3-02D-B auto-close/
...
staging; authenticated browser/API và manual NVDA acceptance cũng PASS, nên P3-02D-A đã
`DONE`.
...
3. Prejoin device/network test và optional effect progressive enhancement theo research decision.
...
P5-COLLAB-07 closure 2026-08-22 — `DONE`: ADR-0035 và migrations `000038`-`000040` thiết lập durable bounded
...
và cho terminal source redaction sau retention purge. Review/no-secret, exact candidate `060fcc7`,
...
P5-COLLAB-09 closure 2026-08-22 — `DONE`: ADR-0037 chấp nhận whiteboard feature default-off,
...
database gate/full verify PASS sau sửa. Review/no-secret PASS; exact candidate `983ada6` đã push lên
`origin/main`, GitHub Verify `32586930775` và Security `32586930710` PASS. Retained media ACL expectation
đã được căn chỉnh từ ledger `40` lên latest ledger `41` trước lần CI xanh cuối. P5-COLLAB-10 runnable.
...
P5-COLLAB-11 checkpoint 2026-08-23 — `DONE`: runtime revalidate exact authority lease sau grant exchange
...
migration/Neon/B2 disposable/shared-staging/deploy; production tiếp tục force-off. Exact candidate `f9d1381`
đã push lên `origin/main`; GitHub Verify `32605702341` và Security `32605702349` đều PASS. P5-COLLAB-12 là
...
P5-COLLAB-13 checkpoint 2026-08-23 — `DONE`: local matrix `7/7`, production artifact regression
...
P5-COLLAB-14 checkpoint 2026-08-23 — `DONE`: real authorized Hocuspocus/Y.Doc profiles
...
production tiếp tục force-off. Full repository verify và final diff/no-secret review PASS. Exact candidate
...
P5-COLLAB-15 closure 2026-08-23 — `DONE`: production semantic fallback có focus handoff và
...
provider/shared-staging/deploy. Exact candidate `09449fc` đã push lên `origin/main`; GitHub Verify
`32638928557` và Security `32638928501` đều PASS. P5-COLLAB-15 chuyển `VERIFY -> DONE`;
...
P5-COLLAB-18 checkpoint 2026-08-24 — `DONE`: workspace `P5-COLLAB-18 Internal Canary` có active
Organization Admin/Teacher/Student, synthetic fixture, exact-one allowlist và low-quota profile.
Shared P5-09/P5-10 PostgreSQL integration cùng exact canary audit xác nhận capability, tenant
isolation, roles/quota/fixture và whiteboard residue zero. Candidate `8d65898`, packaging fix
`ccc1f13`, local/full verify và GitHub Verify/Security đều PASS. Render Core API/control/runtime
healthy/ready; authenticated live metrics có đủ bốn dependency signal và active/dirty/drain đều
zero. Disposable Neon/B2 giữ final `41 false`, `RPO=last_verified_artifact`, `RTO_MS=2042`.
Retained P5-COLLAB-15 Chrome/Edge + NVDA component matrix và P5-COLLAB-16/P5-COLLAB-17 force-off/
outage/export/restore evidence được tái xác nhận bằng P5-18 regression; task không đổi browser
semantics, provider topology hoặc snapshot format. Exact-one canary tenant enabled, tenant khác
...
disposable branch được giữ lại. P4-09 reconnect/recovery/audio-only đã `DONE` trên exact
`fe33ffaba19d8f82f2034ddeb4b16d4e919e5014`: Verify `31868991020`, Security `31868991007`, shared
