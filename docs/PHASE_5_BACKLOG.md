...
> và Security `32566880332` đều PASS. P5-COLLAB-08 đã `DONE` ngày 2026-08-22 trên exact candidate
> `4f05ae6`: reconnect, compaction, terminal recovery, exact Neon/B2 disposable recovery, full verify,
...
> GitHub Verify `32643119744` và Security `32643119716` đều PASS. P5-COLLAB-17 đã `DONE` ngày
...
**Closure 2026-08-22 — DONE:** ADR-0035, forward migrations `000038`-`000040`, durable command/checkpoint/purge
...
compatibility mà không nới invariant của active restore. Review/no-secret, full repository verify và
exact candidate `060fcc7` đã PASS GitHub Verify `32566880308` cùng Security `32566880332`. Không
shared-staging write/deploy; production force-off. P5-COLLAB-08 runnable.
Acceptance:
...
**Checkpoint 2026-08-22 — DONE:** ADR-0036 chấp nhận Yjs state vector làm resume watermark,
...
cũng PASS. Exact candidate `4f05ae6`, GitHub Verify `32571244058` và Security `32571244052` PASS. Không
migration/shared-staging write/deploy; production tiếp tục force-off. P5-COLLAB-09 runnable. Acceptance:
...
**Closure 2026-08-22 — DONE:** ADR-0037 chấp nhận `classroom_whiteboards` default false,
...
phát hiện và sửa trước commit, sau đó database gate/full verify PASS. Review/no-secret PASS; exact candidate
`983ada6` đã push lên `origin/main`, GitHub Verify `32586930775` và Security `32586930710` PASS sau khi một
retained media ACL expectation được căn chỉnh từ ledger `40` lên latest ledger `41`. P5-COLLAB-10 runnable.
Không rollback/shared-staging migration/deploy; production whiteboard tiếp tục force-off và disposable
branch được giữ lại. Acceptance:
...
- [x] Matrix owner/teacher/TA/student/guest/removed/inactive cho lifecycle và `view/edit/present` PASS local.
- [x] IDOR, cross-tenant document/snapshot/export, forged tenant/role/provider ID và stale membership deny
      PASS local và exact PostgreSQL disposable.
...
**Closure 2026-08-23 — DONE:** runtime nay revalidate exact authority lease sau exchange và trước
...
diff/no-secret review PASS. Exact candidate `f9d1381` đã push lên `origin/main`; GitHub Verify
`32605702341` và Security `32605702349` đều PASS. P5-COLLAB-12 là task runnable tiếp theo. Acceptance:
...
- [x] Hai browser concurrent edit, offline/reconnect, duplicate/out-of-order delivery hội tụ.
...
- [x] Immutable checksum snapshot và clean export/import round-trip giữ final document hash.
- [x] Corrupt/incompatible/oversize/malicious import bị quarantine và không phát operation.
...
**Checkpoint 2026-08-23 — DONE:** real authorized Hocuspocus/Y.Doc profiles `2 x 500`, `10 x 500`
...
force-off. Full repository `pnpm verify` và final diff/no-secret review PASS. Exact candidate `e1b1201`
đã push lên `origin/main`; GitHub Verify `32621951917` và Security `32621951910` đều PASS.
P5-COLLAB-14 chuyển `VERIFY -> DONE`; P5-COLLAB-15 là task runnable tiếp theo. Acceptance:
...
- [x] Keyboard-only open/close/tool/object/focus recovery và visible focus PASS.
- [x] Windows Chrome/Edge + NVDA, 200% zoom, forced colors và reduced motion có physical evidence.
...
- [x] Provider/broker/B2/PostgreSQL outage, latency, credential rotation và recovery drill PASS.
- [x] Existing room có behavior công bố; room mới fail closed; content không mất vượt RPO.
...
collaboration authority. Evidence disposable provider 600 giây, B2 rotation và `RTO_MS=3096` từ
...
Closure 2026-08-24 — `DONE`: exact candidate `637e8b5` có GitHub Verify `32649917938` và Security
`32649917980` PASS. Disposable Neon/B2 PASS final `41 false`; shared staging forward
`37 false -> 41 false -> 41 false`, exact ACL/read-only audit PASS. Render deploy exact SHA giữ
deployment force-off; health/readiness/status, authenticated Teacher concealment, privacy boundary,
retained Chrome/Edge + NVDA regression và final zero-residue cleanup snapshot đều PASS. Whiteboard
không được bật; P5-COLLAB-18 internal canary vẫn cần authorization riêng. Acceptance:
[`P5_COLLAB_17_STAGING_ACCEPTANCE.md`](P5_COLLAB_17_STAGING_ACCEPTANCE.md).

**Exit gate:**

- [x] Exact candidate CI/security, disposable migration/ACL và isolated provider gates xanh.
- [x] Shared staging chỉ forward sau disposable report; feature vẫn deployment force-off.
- [x] Authenticated staging, retained Chrome/Edge + NVDA và post-test cleanup snapshot PASS.
...
**Checkpoint chuẩn bị 2026-09-18 - lịch sử:** local contract đã khóa baseline P5-COLLAB-19
...
**Pre-live corrected-candidate checkpoint 2026-09-18:** owner đã phê duyệt authorization hai giai đoạn cho
...
P5-COLLAB-19 regression PASS. Candidate sau đó được commit/push và nhận authorization exact-SHA mới;
production/shared staging vẫn ngoài phạm vi.

**Live R3 checkpoint 2026-09-18 — VERIFY:** exact candidate
`4412bbdad87e425b5cdba00a05abcd8cf6b30008` đã PASS exact-target binding, initial `off`,
evaluator mode, provider preflight và full drill execution trên đúng disposable target. Hai hold đều
đủ 3.600 giây. Lượt 1 có P95 join/reconnect/convergence/ack/artifact
`1229/1474/664/441/2926 ms`; lượt retry cuối là `1349/1793/587/444/6480 ms`. Cả hai chỉ FAIL
artifact p95 so với ngưỡng `2500 ms`; các gate/drill/soak cleanup khác PASS. Six-review completion
packet không được tạo.
