...
- LiveKit staging project và smoke test thật 2-5 trình duyệt: chưa thực hiện vì local
  environment chưa có LiveKit credential.
...
5. Core API kiểm tra `class.view` + `session.join`, sau đó phát JWT TTL 5 phút. Quyền
   publish chỉ bật khi principal có `media.publish`; publish data luôn tắt trong spike.
...
- Enrollment, roster, lịch `ClassSession` và quyền theo từng lớp thuộc phase domain sau.
