...
Trạng thái sau exit gate Phase 1 và cập nhật P2-05:
...
Cập nhật P2-05 ngày 2026-07-19: Core API và web chỉ cho bắt đầu luồng media khi class
đang active và `viewer_access` authoritative cho phép join/publish. Draft/archived hoặc
enrollment inactive bị chặn trước khi phát token hay ghi media event mới. Source hiện có
migration `000010_class_enrollments`; integration-tag compile xanh local nhưng runtime
`000010` chưa chạy local vì không nạp DB test env. Bằng chứng staging gần nhất vẫn là
`5 false`, không được hiểu là staging đã nâng lên version 10.
...
5. Core API resolve class và enrollment persisted, chỉ nạp class role khi enrollment
   `active`, rồi kiểm tra class active và `viewer_access.can_join_room`. JWT có TTL 5
   phút; publish chỉ bật khi `viewer_access.can_publish_media`, không suy từ quyền join.
   Owner implicit và organization role vẫn phải qua shared policy; publish data luôn tắt.
...
| Enrollment inactive | Student `suspended/left/removed` gọi token/event | Không có class/media access, không phát JWT/ghi event |
...
Endpoint token và telemetry cùng đọc trạng thái class/enrollment authoritative;
draft/archived hoặc enrollment inactive không được bắt đầu hay tiếp tục request media
mới. Web dùng riêng `can_join_room` và `can_publish_media`, không dùng permission tenant
cũ hoặc tự suy publish từ join. Archive không thu hồi JWT LiveKit đã cấp và không kick
participant đang ở trong room; JWT ngắn hạn giảm cửa sổ hiệu lực, còn revoke/kick chủ
động thuộc moderation/session lifecycle về sau.
...
- P2-05 đã cung cấp enrollment, class invite code và projection quyền theo từng lớp.
  Roster search/phân trang, UI đổi role đầy đủ, lịch `ClassSession` và moderation/session
  lifecycle vẫn thuộc các task sau.
