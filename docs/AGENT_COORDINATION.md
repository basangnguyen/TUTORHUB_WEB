...
| CI trên GitHub           | `Verify` và `Security` của PR #4 thành công ngày 2026-07-15       |
...
| P1-04 Core API foundation    | DONE       | Codex       | `codex/p1-04-core-api-foundation` | `services/core-api`, OpenAPI và tài liệu task liên quan                                                     | 2026-07-13 | Đã merge vào `main` qua PR #4 tại `82261c6`; unit test, runtime smoke và `pnpm verify` đạt                                                       |
| P1-05 Contract và PostgreSQL | DONE       | Codex       | `codex/p1-05-contract-database`   | `openapi`, `packages/api-client`, database platform/migrations/repository, CI và tài liệu task              | 2026-07-13 | Đã merge qua PR #4; generated client, migration v5, Neon integration và runtime smoke đạt; credential chỉ đọc từ `.env.local`                    |
| P1-06 Authentication         | DONE       | Codex       | `codex/p1-06-authentication`      | ADR/auth docs, config, identity module/migration, HTTP auth routes, OpenAPI/generated client và web session | 2026-07-13 | Đã merge qua PR #4; `tutorhub-local` login/session/reload/CSRF/logout browser smoke đạt; staging deferred đến P1-10                              |
| P1-06A Workspace onboarding  | DONE       | Codex       | `codex/p1-workspace-onboarding`   | Identity tenant/session service, workspace HTTP routes, OpenAPI/client, web onboarding/selector và tài liệu | 2026-07-14 | Đã merge qua PR #4; tạo workspace, `org_admin`, active tenant, session/CSRF rotation và tenant-isolation tests đạt                               |
| P1-06B Class vertical slice  | DONE       | Codex       | `codex/p1-class-vertical-slice`   | Classroom service/policy/repository tests, HTTP API, OpenAPI/client, web list/create/detail và task docs    | 2026-07-14 | Đã merge qua PR #4; unit/HTTP/client/web/Neon test và runtime smoke đạt; tenant lấy từ authenticated active session                              |
...
| P1-08A CI/security baseline  | DONE       | Codex       | `codex/p1-08a-ci-security`        | `.github/**`, security scripts/policy, migration guards, ADR/runbook và state docs                          | 2026-07-15 | PR #4 merge tại `82261c6`; Verify/Security xanh, CODEOWNERS và Dependabot hoạt động; ruleset evidence là follow-up quản trị, deploy thuộc P1-08B |
...
| P1-03 Design system              | DONE       | Token/component catalog, Storybook, accessibility, contrast, visual QA và tích hợp web đã đạt |
| P1-04 Go Core API                | DONE       | Đã merge qua PR #4; API foundation, test và runtime smoke đạt                                 |
| P1-05 Contract và database       | DONE       | Đã merge qua PR #4; runtime/migration role tách tiếp ở P1-10                                  |
| P1-06 Authentication             | DONE       | Đã merge qua PR #4; local browser smoke đạt, staging client deferred đến P1-10                |
| P1-06A Workspace onboarding      | DONE       | Đã merge qua PR #4; tạo/chọn workspace, tenant isolation và session rotation đạt              |
| P1-06B Class vertical slice      | DONE       | Đã merge qua PR #4; list/create/detail, permission, contract, UI states và deny test đạt      |
