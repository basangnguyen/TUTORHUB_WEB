...
| P2-10 Tenant isolation/IDOR    | VERIFY     | Code/local verify xanh; chờ PostgreSQL CI và security scan  |
...
security fixture; full `corepack pnpm verify` xanh cục bộ ngày 2026-07-22. Task đang
`VERIFY` vì host không có PostgreSQL/Docker; phải chờ Verify/Security workflow trên cùng
head trước khi chuyển `DONE` và mở P2-11.
